...
export default function Home() {
  const { user, profile, loading, loginWithGoogle, loginAnonymous, logout, updatePrefs } = useAuth()
  ...
  const {
    ...
    fetchSessions, newSession, removeSession, updateSessionLocal,
  ...
  const handleSend = async (content: string) => {
    ...
    await sendMessage(content, (sessionId) => {
      updateSessionLocal(sessionId, {
        ...
        ...(isFirstMsg ? { title: content.slice(0, 50) + (content.length > 50 ? "..." : "") } : {}),
  ...
  if (!user) {
    return <LoginScreen onGoogle={loginWithGoogle} onAnonymous={loginAnonymous} />
  }
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: "var(--bg-primary)" }}>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        >
          ...
          {/* Logo + title */}
          <img src="/logo.svg" alt="AI Advisor" className="w-8 h-8 rounded-xl shrink-0" />
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-sm truncate" style={{ color: "var(--text-primary)" }}>
              {sessions.find((s) => s.id === activeSessionId)?.title || "AI Advisor"}
            </h1>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {error && (
            <div className="text-center text-xs py-2 px-4 mx-auto max-w-sm rounded-lg"
